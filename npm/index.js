...
class wordsearchWrapper {
    ...
    constructor(options) {
        if (!options || typeof options !== 'object') {
            throw new Error('Options object must be provided. See documentation: https://docs.apiverve.com/ref/wordsearch');
        }
    ...
    async execute(query, callback) {
        ...
        var requiredParams = ["words"];
        if (requiredParams.length > 0) {
            for (var i = 0; i < requiredParams.length; i++) {
                if (!query[requiredParams[i]]) {
                    throw new Error(`Required parameter [${requiredParams[i]}] is missing. See documentation: https://docs.apiverve.com/ref/wordsearch`);
                }
            }
        }
...
module.exports = wordsearchWrapper;
